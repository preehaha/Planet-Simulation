...
class Planet:
    AU = 149.6e6 * 1000  # astronomical units in metres
    ...
    def __init__(self, x, y, radius, color, mass):
        self.x = x
        self.y = y  # metres away from sun (x & y)
        self.radius = radius
        self.color = color
        self.mass = mass

        self.orbit = []
        self.sun = False  # no orbit for sun
    ...
    def draw(self, win):
        x = self.x * self.SCALE + WIDTH / 2  # centre of screen
    ...
    def attraction(self, other):  #force of attraction between 2 objects
...
def main():
    run = True
    clock = pygame.time.Clock()  # to synchronize game
    ...
    while run:
        clock.tick(60)  # no. of updates per second
        WIN.fill((0, 0, 0))
